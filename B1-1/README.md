...
프로젝트 데이터는 [`js/projects-data.js`](js/projects-data.js)에 배열로 분리해 두고,
`map()`과 템플릿 리터럴로 카드를 만들어 넣습니다.
카드 전체가 링크(`<a>`)라서 클릭하면 해당 프로젝트의 Notion 상세 문서로 이동합니다.

| 프로젝트 | 소속 | 기간 | 역할 |
|---|---|---|---|
| **SuitdiO** — AI 논리 에디터 | SuitdiO | 2024.06 – 2024.12 | Co-Founder · 기획 & 프론트엔드 |
| **마일퀘** — 마케터를 위한 스크랩 서비스 | 팬로즈 | 2024.08 – 2024.10 | Co-Founder · 기획 & 프론트엔드 |
| **축구관** — 해외축구 단체관람 서비스 | 축구관 | 2023.10 – 2024.05 | 대표 · 서비스 총괄 |
| **잠시맘** — 아이돌봄 서비스 | 멋사 스타트업 스쿨 · Oblet | 2022 | Co-Founder · Product Manager |

경력과 프로젝트가 사실상 같은 내용이라 **하나의 카드 목록으로 합쳤습니다.**
각 카드에 기간을 뱃지로 표시해, 별도의 경력 타임라인 없이도 시간 흐름을 알 수 있게 했습니다.

카드를 `<div>` + JS 클릭 핸들러가 아니라 **`<a>` 태그로 만든 이유**는,
그렇게 해야 브라우저가 기본으로 제공하는 것들(Tab 이동, Enter로 열기, 새 탭으로 열기,
링크 주소 복사)을 직접 구현하지 않고 그대로 쓸 수 있기 때문입니다.
