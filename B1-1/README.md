...
| **잠시맘** — 단시간 아이 돌봄 매칭 서비스 | Oblet · 멋사 스타트업 스쿨 | 2022.08 – 2022.10 | 기획 · Product Manager |
...
│   └── projects-data.js    # Work 섹션 프로젝트 데이터
├── images/
│   └── profile.png         # 프로필 이미지
...
| 5. Work | 프로젝트 카드 렌더링 (Notion 문서로 연결) |
| 6. Projects | GitHub API 호출, 상태별 렌더링, 언어 필터 |
| 7. Contact 폼 | 유효성 검사, 메일 전송 |
| 8. 타이핑 효과 | Hero 이름 한 글자씩 출력 |

데이터를 `projects-data.js`로 따로 뺀 이유는, **내용을 고칠 때 로직 코드를 건드리지 않기 위해서**입니다.
프로젝트를 추가하려면 배열에 객체 하나만 넣으면 되고 렌더링 코드는 그대로입니다.
...
### 카드를 `<div>`가 아니라 `<a>`로 만든 이유

처음에는 카드를 `<div>`로 만들고 JS로 클릭 이벤트를 붙였습니다. 그러다 보니
키보드 접근을 위해 `tabindex`, `role="button"`, Enter/Space 처리를 직접 다 넣어야 했습니다.

카드 자체를 `<a>`로 바꾸자 그 코드가 전부 필요 없어졌습니다.
Tab 이동, Enter로 열기, 새 탭으로 열기(⌘+클릭), 링크 주소 복사가 전부 브라우저 기본 동작으로 해결됩니다.
**직접 구현하기 전에 표준 태그가 이미 해주는 일인지 먼저 확인해야 한다**는 걸 배웠습니다.

다만 `<a>` 안에 또 다른 `<a>`를 넣을 수는 없어서, 링크가 두 개 필요한 경우에는
카드를 `<article>`로 두고 안에 링크를 나란히 배치하는 방식으로 나눠야 합니다.
