...
과제 데이터는 최대 25×25 지만, 실제 AI 모델은 훨씬 큰 패턴을 다룹니다.
1000×1000 으로 늘렸을 때 무엇이 문제가 되는지 **직접 측정해** 확인했습니다.

### 6-1. 실측 데이터

크기를 키워가며 잰 값입니다. 마지막 줄은 실제로 1000×1000 을 돌린 결과입니다.

| 크기 | 연산 횟수 | 평균 시간 | 25×25 대비 연산 | 25×25 대비 시간 |
|---|---|---|---|---|
| 25×25 | 625 | 0.0189 ms | 1배 | 1배 |
| 50×50 | 2,500 | 0.0712 ms | 4배 | 3.8배 |
| 100×100 | 10,000 | 0.2815 ms | 16배 | 14.9배 |
| 200×200 | 40,000 | 0.9954 ms | 64배 | 52.7배 |
| **1000×1000** | **1,000,000** | **27.5 ms** | **1,600배** | **1,455배** |

200×200 까지의 실측으로 1000×1000 을 외삽하면 **24.9 ms** 가 나오는데, 실제로
재보니 **27.5 ms** 였습니다. 오차 10% 안쪽으로, **O(N²) 예측이 실제와 맞아떨어집니다.**
크기가 커질수록 고정 비용 비중이 줄어 이론값에 더 가까워지는 것도 확인됩니다.

### 6-2. 시간 병목

패턴 한 장에 27.5 ms 면 그 자체로는 견딜 만합니다. 문제는 **곱해지는 요소**입니다.

| 상황 | 예상 시간 |
|---|---|
| 패턴 1장 × 필터 1개 | 27.5 ms |
| 패턴 1장 × 필터 2개 (이 프로그램) | 약 55 ms |
| 패턴 1장 × 필터 1,000개 (실제 AI 모델 수준) | **약 27.5초** |

실제 모델은 이런 연산을 층층이 쌓으므로, 이미지 한 장 처리에 수십 초가 걸리는
셈입니다. **근본 원인은 순수 파이썬 반복문**입니다. 곱셈 100만 번을 인터프리터가
한 줄씩 해석하며 실행하기 때문에, C 로 짠 코드보다 수십~수백 배 느립니다.

### 6-3. 메모리 병목

1000×1000 격자 하나를 잡고 실제로 잰 값입니다.

| 항목 | 크기 |
|---|---|
| 2차원 리스트 구조 자체 | 7.69 MB |
| 1차원 리스트 구조 자체 | 8.06 MB |
| float 객체 100만 개 | 약 23 MB |

**숫자 하나가 8바이트가 아니라 24바이트를 씁니다.** 파이썬의 float 는 단순한 값이
아니라 객체라서 타입 정보와 참조 카운터를 함께 들고 다니기 때문입니다. 게다가
리스트는 값이 아니라 **각 객체를 가리키는 포인터**를 저장하므로, 값이 메모리에
흩어져 있어 캐시 적중률도 떨어집니다.

### 6-4. 최적화 우선순위

**1순위 — 1차원 배열 변환** (이미 구현, 5-1 참고)

코드 구조를 크게 바꾸지 않으면서 효과가 확실합니다. 1000×1000 에서도 실측했습니다.

```
2차원: 27.5 ms  →  1차원: 21.8 ms   (20.7% 단축)
```

인덱싱이 한 단계 줄고 값이 연속 배치되어 캐시 효율이 오릅니다. 다만 이건 **상수 배
개선**이지 복잡도를 바꾸지는 못합니다. 여전히 N² 개를 모두 훑어야 합니다.

**2순위 — 불필요한 곱셈 건너뛰기**

필터는 대부분 0 입니다. 1000×1000 십자가 필터에서 1인 칸은 약 2,000개로 **전체의
0.2%** 뿐입니다. 0을 곱하면 결과가 0이라 누적에 영향이 없으므로, 0이 아닌 위치만
미리 추려두면 연산량 자체를 크게 줄일 수 있습니다. 다만 필터가 조밀하면 효과가
없으므로 데이터 특성에 의존하는 최적화입니다.

**3순위 — 벡터화 라이브러리 / 병렬 처리**

NumPy 는 같은 연산을 C 수준에서 처리하고 여러 값을 한 번에 계산합니다. 그 이상은
GPU·NPU 로 넘어갑니다. 이 과제는 **외부 라이브러리 금지**라 적용하지 않았고,
MAC 원리를 직접 구현하는 것이 목적이므로 여기서는 범위 밖입니다.

### 6-5. CPU 직렬 처리와 NPU 병렬 처리

| 구분 | CPU | NPU |
|---|---|---|
| 설계 목적 | 복잡한 명령을 순서대로 빠르게 | 단순 연산을 동시에 대량으로 |
| 코어 수 | 수 개 ~ 수십 개 | 수천 개 연산 유닛 |
| 1000×1000 MAC | 100만 번을 차례로 실행 | 대량을 한 번에 처리 |
| 이론적 소요 | O(N²) 에 비례 | 유닛 수만큼 나눠져 크게 단축 |

**병목이 어디에 생기는가**: 이 프로그램의 이중 for문이 정확히 CPU 직렬 처리의
모습입니다. 100만 번의 곱셈이 서로 **의존하지 않는데도** 한 줄씩 순서대로 실행됩니다.
1번째 칸의 곱셈 결과가 2번째 칸 계산에 필요하지 않은데도 기다리는 셈입니다.

NPU 는 이 지점을 노립니다. **서로 독립적인 곱셈들을 회로 차원에서 동시에** 수행하고,
그 결과를 한꺼번에 더합니다. 병목이 "연산량 자체"가 아니라 "순차 실행"이었기 때문에,
연산 방식을 바꾸지 않고 실행 구조만 바꿔도 큰 이득이 납니다. AI 전용 칩이 등장한
이유가 바로 이것입니다.
...
`main.py` 는 외부 라이브러리 없이 표준 라이브러리만 사용하므로 별도 설치 과정이 없습니다.
`data.json` 은 `main.py` 와 같은 폴더에 있어야 모드 2가 동작합니다.
